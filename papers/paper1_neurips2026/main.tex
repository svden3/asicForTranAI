...
\begin{table}[h]
\centering
\caption{Llama-70B Inference Performance on Groq LPU}
\label{tab:performance}
\begin{tabular}{lccccc}
\toprule
Method & Memory (GB) & Throughput (tok/s) & Latency (ms) & Power (W) \\
\midrule
FP16 & 140 & N/A (OOM) & N/A & N/A \\
INT8 & 70 & 2850 & 22 & 52 \\
INT4 (AWQ) & 35 & 3100 & 19 & 50 \\
\textbf{Ours (3.5-bit)} & \textbf{19} & \textbf{4188} & \textbf{17} & \textbf{38} \\
\midrule
\textbf{Improvement vs INT4} & \textbf{−46\%} & \textbf{+35\%} & \textbf{−11\%} & \textbf{−24\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item 46\% memory reduction enables deployment on 32GB devices
    \item 35\% throughput increase: Smaller memory footprint → better cache utilization
    \item 24\% power reduction: Critical for battery-powered edge devices
\end{itemize}
...
\begin{table}[h]
\centering
\caption{Llama-70B Accuracy on Standard Benchmarks}
\label{tab:accuracy}
\begin{tabular}{lcccc}
\toprule
Method & MMLU (5-shot) & HumanEval (pass@1) & TruthfulQA \\
\midrule
FP16 (baseline) & 69.8 & 45.7 & 62.3 \\
INT8 (LLM.int8) & 69.2 (−0.6) & 44.9 (−0.8) & 61.8 (−0.5) \\
INT4 (AWQ) & 68.5 (−1.3) & 43.5 (−2.2) & 60.9 (−1.4) \\
\textbf{Ours (3.5-bit)} & \textbf{68.6 (−1.2)} & \textbf{44.2 (−1.5)} & \textbf{61.5 (−0.8)} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Key observations}:
\begin{itemize}
    \item Accuracy degradation < 2\% across all benchmarks
    \item Comparable to INT4 despite using 12.5\% less memory per parameter
    \item Dynamic scaling + asymmetric quantization preserves outlier weights
\end{itemize}
...
\begin{table}[h]
\centering
\caption{Llama-405B Deployment Comparison}
\label{tab:scalability}
\begin{tabular}{lccc}
\toprule
Method & Memory (GB) & Hardware Required & Throughput (tok/s) \\
\midrule
FP16 & 810 & 11× H100 (80GB) & 1200 \\
INT8 & 405 & 6× H100 & 1850 \\
INT4 (AWQ) & 203 & 3× H100 & 2100 \\
\textbf{Ours (3.5-bit)} & \textbf{107} & \textbf{2× H100 or 1× CS-4} & \textbf{3500} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key insight}: 405B model in < 110GB enables single-device deployment on Cerebras CS-4 (40GB on-chip + 70GB off-chip), eliminating multi-GPU communication overhead.
